...
//  2. Whenever there is a running process or the FCFS queue is not empty:
        ...
        while (input_queue || current_process || priority_queue_1 || priority_queue_2 || priority_queue_3) 
        {
            while (input_queue && input_queue->arrival_time <= timer) {
                PcbPtr deq_process = deqPcb(&input_queue);

                switch (deq_process->priority) {
                    case 1:
                    priority_queue_1 = enqPcb(priority_queue_1, deq_process);
                    break;
                    case 2:
                    priority_queue_2 = enqPcb(priority_queue_2, deq_process);
                    break;
                    case 3:
                    priority_queue_3 = enqPcb(priority_queue_3, deq_process);
                    break;
                }
            }
